        ...
        if (error) {
           callback(null, error);
            ...
            if (data.hasOwnProperty('body')) {
                ...
                var dataArrayCount = dataArray.result.length;
                var changeTicket = [];

                for(var index = 0; index < dataArrayCount; index += 1) {
     ...
     */
      this.connector.post((data, error) => {
        ...
        if (error) {
            ...
            if( data.hasOwnProperty('body') ) {
                var dataResult = (JSON.parse(data.body).result);
                var changeTicket = {};

                changeTicket = ({   "change_ticket_number" : dataResult.number, 
                                    "active" : dataResult.active, 
                                    "priority" : dataResult.priority,
                                    "description" : dataResult.description,
                                    "work_start" : dataResult.work_start,
                                    "work_end" : dataResult.work_end,
                                    "change_ticket_key" : dataResult.sys_id
                                });
                callback( changeTicket, error); 
            }
        }
